...
<body>
    ...
    <?php
// Retrieve the data
$sql = "SELECT * FROM faculty_feedback_system.view1";
...
// $sql = "SELECT DISTINCT rollno FROM faculty_feedback_system.view1 ORDER BY rollno ASC";
// $result = mysqli_query($conn, $sql);
...
$sql = "SELECT DISTINCT facultyname FROM view1 ORDER BY facultyname ASC";
$result = mysqli_query($conn, $sql);
$facultyname_options = "<option value=''>All Faculty's</option>"; // Add an option to select all rows
while ($row = mysqli_fetch_assoc($result)) {
    $selected = ($row["facultyname"] == $_POST["facultyname_filter"]) ? "selected" : ""; // Check if this option is selected
    $facultyname_options .= "<option value='" . $row["facultyname"] . "' $selected>" . $row["facultyname"] . "</option>";
}
...
echo "<label>Faculty Name:</label>";
echo "<select name='facultyname_filter'>";
echo $facultyname_options;
echo "</select>";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $facultyname_filter = $_POST["facultyname_filter"];
    ...
    $sql = "SELECT * FROM view1 WHERE facultyname LIKE '%$facultyname_filter%'
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        echo "<tr><td>" . $row["rollno"] . "</td><td>" . $row["facultyname"] . "</td><td>" .
...
?>
    ...
    function clearFilters() {
        ...
        document.getElementsByName("facultyname_filter")[0].value = "";
