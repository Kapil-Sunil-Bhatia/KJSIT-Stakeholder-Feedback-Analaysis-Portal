...
<body>
    ...
    <?php
    // Retrieve the data
    $sql = "SELECT * FROM parent_feedback_system.view1";
    ...
    $sql = "SELECT DISTINCT rollno FROM parent_feedback_system.view1 ORDER BY rollno ASC";
    $result = mysqli_query($conn, $sql);
    ...
    $sql = "SELECT DISTINCT parentname FROM view1 ORDER BY parentname ASC";
    $result = mysqli_query($conn, $sql);
    $parentname_options = "<option value=''>All Parent's</option>"; // Add an option to select all rows
    while ($row = mysqli_fetch_assoc($result)) {
        $selected = ($row["parentname"] == $_POST["parentname_filter"]) ? "selected" : ""; // Check if this option is selected
        $parentname_options .= "<option value='" . $row["parentname"] . "' $selected>" . $row["parentname"] . "</option>";
    }
    ...
    echo "<label>Parent Name:</label>";
    echo "<select name='parentname_filter'>";
    echo $parentname_options;
    echo "</select>";
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $parentname_filter = $_POST["parentname_filter"];
        ...
        $sql = "SELECT * FROM view1 WHERE rollno LIKE '%$rollno_filter%' AND parentname LIKE '%$parentname_filter%'
     ...
     AND Department LIKE '%$Department_filter%' AND academicyear LIKE '%$academicyear_filter%' AND feedback_topic LIKE '%$feedback_topic_filter%' ORDER BY rollno";
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            echo "<tr><td>" . $row["rollno"] . "</td><td>" . $row["parentname"] . "</td><td>" .
    ...
    function clearFilters() {
        ...
        document.getElementsByName("parentname_filter")[0].value = "";
