...
<body>
    ...
    <?php
// Retrieve the data
$sql = "SELECT * FROM peers_feedback_system.view1";
...
// $sql = "SELECT DISTINCT rollno FROM peers_feedback_system.view1 ORDER BY rollno ASC";
// $result = mysqli_query($conn, $sql);
...
$sql = "SELECT DISTINCT peersname FROM view1 ORDER BY peersname ASC";
$result = mysqli_query($conn, $sql);
$peersname_options = "<option value=''>All peers's</option>"; // Add an option to select all rows
while ($row = mysqli_fetch_assoc($result)) {
    $selected = ($row["peersname"] == $_POST["peersname_filter"]) ? "selected" : ""; // Check if this option is selected
    $peersname_options .= "<option value='" . $row["peersname"] . "' $selected>" . $row["peersname"] . "</option>";
}
...
echo "<label>peers Name:</label>";
echo "<select name='peersname_filter'>";
echo $peersname_options;
echo "</select>";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $peersname_filter = $_POST["peersname_filter"];
    ...
    $sql = "SELECT * FROM view1 WHERE peersname LIKE '%$peersname_filter%'
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        echo "<tr><td>" . $row["rollno"] . "</td><td>" . $row["peersname"] . "</td><td>" .
...
?>
    ...
    function clearFilters() {
        ...
        document.getElementsByName("peersname_filter")[0].value = "";
